<?php
// ARQUIVO: admin_save_card.php

// --- 1. FUNÇÃO PARA CARREGAR O .ENV (Se rodar local, no Render ignora) ---
...
if (!isset($data['email']) || !isset($data['token'])) {
    ...
    echo json_encode(['error' => 'Email do admin e token do cartão são obrigatórios.']);
    exit();
}
...
if (isset($search_data['results']) && count($search_data['results']) > 0) {
    // Cliente/Admin já existe
    $customer_id = $search_data['results'][0]['id'];
} else {
    // Admin não existe como customer, vamos criar
    $customer_data = [
        "email" => $email
    ];
